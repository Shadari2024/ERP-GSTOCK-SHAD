...
class UtilisateurPersonnalise(AbstractUser):
 ...
 # Champ pour les admins SaaS
    ...
    def get_assigned_pos(self):
        """Retourne tous les POS assignés à ce caissier"""
        if hasattr(self, 'pos_caissiers'):
            return self.pos_caissiers.all()
        from ventes.models import PointDeVente
        return PointDeVente.objects.none()
    
    def get_main_pos(self):
        """Retourne le POS où il est responsable"""
        if hasattr(self, 'pos_responsable'):
            return self.pos_responsable.first()
        return None
    
    def get_accessible_pos(self):
            """Retourne tous les POS accessibles par l'utilisateur"""
            if self.role == 'CAISSIER':
                # Pour les caissiers, retourner seulement leurs POS assignés
                return self.get_assigned_pos()
            elif self.role in ['ADMIN', 'MANAGER']:
                # Pour les admins/managers, retourner tous les POS de l'entreprise
                from ventes.models import PointDeVente
                return PointDeVente.objects.filter(entreprise=self.entreprise)
            else:
                # Pour les autres rôles, retourner aucun POS
                from ventes.models import PointDeVente
                return PointDeVente.objects.none()
        
    def can_access_pos(self, pos):
            """Vérifie si l'utilisateur peut accéder à un POS spécifique"""
            if self.role in ['ADMIN', 'MANAGER']:
                return pos.entreprise == self.entreprise
            elif self.role == 'CAISSIER':
                return pos in self.get_assigned_pos()
            return False
